...
class PDF extends PDF_MC_Table
{
    ...
    function Content()
    {
        ...
        $data_peminjam = select("SELECT * FROM tb_peminjaman p JOIN tb_buku b ON p.no_buku = b.no_buku JOIN tb_koleksibuku kb ON kb.id_buku = b.id_buku JOIN tb_anggota a ON p.nomorAnggota = a.nomorAnggota WHERE p.tgl_pinjam BETWEEN '$tgl_awal' AND '$tgl_akhir'");
        ...
        $this->setWidths(array(8,20,60,25,50,30,35,35));
        $this->SetLineHeight(6);
        $this->SetFont('Arial','B',16);
        $this->Cell(0,5,'Laporan Data Peminjaman',0,1,'C');
        $this->Ln(2);
        $this->SetFont('Arial','i','10');
        $this->Cell(0,5,'Periode :'.$tgl_awal.' S/D '.$tgl_akhir,0,1,'C');
        //$this->Ln(2);
		//$this->SetFont('Arial','i','10');
		//$this->Cell(0,5,'Jumlah Peminjam : '.$jumlahpeminjam,'C');
		//$this->ln();
		//$this->Cell(0,5,'Belum dikembalikan : '.$blmdikembalikan,'C');
        $this->Ln(7);
        ...
        $this->Cell(30,6,'Tanggal Pinjam',1,0,'C');
        $this->Cell(35,6,'Estimasi Tgl kembali',1,0,'C');
        $this->Cell(35,6,'Status Pinjam',1,0,'C');
        $this->Ln();
        ...
        foreach ($data_peminjam as $peminjam){
            $this->Row(array(
                ...
                $peminjam['nama'],
                $peminjam['tgl_pinjam'],
                $peminjam['estimasi_tglkembali'],
                $peminjam['status_pinjam'],
